...
import { BookOpen, Search, ChevronDown, ChevronUp, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-sm border border-gray-100 dark:border-gray-700 text-center">
        ...
        <form onSubmit={handleDiseaseSearch} className="max-w-md mx-auto relative">
          <input 
            type="text" 
            value={searchDisease}
            ...
            placeholder="Search for a disease (e.g. Malaria)..."
          />
          <button 
            type="submit" 
            disabled={loading}
        ...
        {diseaseInfo && (
            <div className="mt-8 text-left bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
                <h3 className="text-xl font-bold text-brand-700 dark:text-brand-300 mb-2 capitalize">{diseaseInfo.name}</h3>
                <p className="text-gray-700 dark:text-gray-300 mb-4">{diseaseInfo.description}</p>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm">
                        <h4 className="font-semibold text-red-500 mb-2">Symptoms</h4>
                        <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-400 space-y-1">
                            {diseaseInfo.symptoms?.map((s, i) => <li key={i}>{s}</li>)}
                        </ul>
                    </div>
                    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm">
                        <h4 className="font-semibold text-green-500 mb-2">Prevention</h4>
                         <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-400 space-y-1">
                            {diseaseInfo.prevention?.map((s, i) => <li key={i}>{s}</li>)}
                        </ul>
                    </div>
                    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm">
                        <h4 className="font-semibold text-blue-500 mb-2">Treatment</h4>
                         <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-400 space-y-1">
